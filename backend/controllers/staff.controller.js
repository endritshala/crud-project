...
const staff_register = (req, res) => {
  ...
  Staff.findOne({ email }).then((staff) => {
    if (staff) return res.status(409).json({ msg: "Email already exists" });
  });
  ...
  bcrypt.genSalt(10, (err, salt) => {
    bcrypt.hash(newStaff.password, salt, (err, hash) => {
      if (err) return res.status(400).json({ msg: "Invalid data recieved" });
...
const staff_get = async (req, res) => {
  ...
  try {
    const data = await Staff.findById(id).populate({
      path: "department",
